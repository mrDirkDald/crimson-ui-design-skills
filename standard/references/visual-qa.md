...
For redesigns informed by external web references:
- confirm the output did not inherit one reference's section order + asset treatment + motion language together;
- confirm high-cost award-style effects have a subject or task reason;
- confirm product flows were evaluated beyond a single happy-path screenshot;
- confirm common gallery trends are not being used solely because they are fashionable;
- confirm typography, layout and motion have been adapted into one product-specific grammar.

Use `web-reference-corpus.md` for the full source-bias and triangulation protocol.
