...
## Lucide default implementation

When no project icon library exists, prefer Lucide for general-purpose web UI.

For React/TypeScript, `lucide-react` is the preferred implementation when dependency changes are allowed.
...
- prefer `currentColor`;
- adjacent text normally provides the accessible label, so mark the icon decorative;
- icon-only controls need an accessible name on the control;
- do not mix Lucide with emoji or arbitrary icon packs.

If adding a dependency is not appropriate, use existing project SVGs or compatible custom SVG instead of emoji.
