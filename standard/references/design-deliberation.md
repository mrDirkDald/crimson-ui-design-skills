...
For web work, do not let every reference answer the same visual question. When research depth is justified, map references to distinct roles such as art direction, flow/usability, typography and implementation. See `web-reference-corpus.md`.

A reference is weak if its mechanism cannot be translated into a product-specific reason.
