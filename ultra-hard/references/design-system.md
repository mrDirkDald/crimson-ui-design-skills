...
## Lucide baseline icon system

If the product has no established coherent icon system, default general-purpose UI icons to **Lucide**.

Use a central icon wrapper/registry where the stack benefits from one.

Define:
- default sizes;
- stroke width policy;
- `currentColor` behavior;
- decorative vs semantic handling;
- accessible naming strategy;
- disabled/active treatment.

Do not mix Lucide with unrelated icon packs casually.

Use custom SVG only when the icon is brand-specific, product-specific, or Lucide does not express the concept accurately.


## Component API pressure test

For shared React/component systems, flag boolean-prop proliferation and impossible combinations.

Prefer explicit variants, composition/slots, compound components, and clear state ownership when they reduce ambiguity.

Before abstracting, ask whether the component is genuinely shared and whether the API models one coherent job.

Do not create abstraction layers for one-off UI merely to appear systematic.
