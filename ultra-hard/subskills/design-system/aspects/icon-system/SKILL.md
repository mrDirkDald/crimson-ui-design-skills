---
name: crimson-ui-design-system-icon-system
description: Deep aspect subskill for icon system within the Crimson UI Design MAX system.
---
...
Standardize icon source, sizing, semantics, and accessibility across the product.

This is a **deep aspect subskill**. Use it after loading the parent `design-system` skill when this aspect materially affects the result.
...
Preserve existing coherent icon systems; otherwise default to Lucide where technically appropriate.
...
Wrap icon behavior centrally when the stack supports it.
...
Define sizes, optical alignment, stroke/fill behavior, semantic color inheritance, and decorative semantics.
...
Use custom SVG for product-specific concepts rather than forcing an inaccurate generic glyph.
...
Never use emoji as a UI icon fallback.
...
- [ ] sizes tokenized
- [ ] SVG normalized
- [ ] Lucide/custom policy clear
- [ ] a11y semantics correct
