---
name: crimson-ui-tauri-commands-ipc
description: Deep aspect subskill for tauri commands and ipc within the Crimson UI Design MAX system.
---
...
Design typed command invocation and asynchronous UI state around Tauri's frontend/native boundary.

This is a **deep aspect subskill**. Use it after loading the parent `tauri` skill when this aspect materially affects the result.
...
Expose narrow commands aligned to product actions rather than generic arbitrary native execution.
...
Model pending, success, failure, cancellation, and retry states where appropriate.
...
Validate privileged inputs in Rust/native code, not only in the frontend.
...
Do not block the UI waiting for long native work when progress/background execution is possible.
...
Keep serialization contracts stable and explicit.
...
- [ ] commands narrow
- [ ] async states modeled
- [ ] native validation present
- [ ] long work nonblocking
- [ ] contracts explicit
