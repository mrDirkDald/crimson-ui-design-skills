---
name: crimson-ui-tauri-windows-state
description: Deep aspect subskill for windows and application state within the Crimson UI Design MAX system.
---
...
Coordinate Tauri windows, dialogs, state synchronization, and background tasks.
...
Define authoritative state ownership across windows/frontend/native backend.
...
Avoid duplicated unsynchronized state when several windows observe the same resource.
...
Restore focus and selection after opening/closing secondary windows.
...
Persist window geometry cautiously across monitor changes.
...
Surface background task progress in the most relevant window/context.
...
- [ ] multi-window sync safe
- [ ] focus restoration
- [ ] geometry recovery
- [ ] background progress placed
