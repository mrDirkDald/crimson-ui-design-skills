...
Separate visual components from data/state where practical. Preserve server/client boundaries. Use semantic components, stable keys, predictable loading/error states, route transitions that respect browser history, and design tokens through CSS variables/theme utilities. Prefer lucide-react when no coherent icon system exists.
