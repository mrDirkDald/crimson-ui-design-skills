# Rust UI Implementation Guide — HARD

## Purpose

Use this guide when UI code is materially implemented in **Rust**. Typical ecosystems include **egui, iced, Slint, Tauri, Ratatui, custom wgpu UI**. The language alone does not determine the UI architecture; inspect the actual toolkit and version before applying APIs.

## Primary engineering concern

Pay special attention to **ownership/message models, async-runtime boundaries, retained vs immediate mode**.
