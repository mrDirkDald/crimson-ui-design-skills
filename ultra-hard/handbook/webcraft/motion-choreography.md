...
This HARD handbook chapter is a deep operational guide for **motion choreography**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Define a motion grammar before individual animations: duration bands, easing family, spatial direction, enter/exit logic, and interruption behavior.
2. Use motion to reveal hierarchy, continuity, causality, and product behavior; avoid movement whose only purpose is to appear premium.
3. Coordinate scroll, hover, cursor, media, and page transitions so they feel like one system rather than plugins from different demos.
4. Keep content accessible and understandable with motion disabled or reduced.
5. Audit CPU/GPU cost, input responsiveness, scroll jank, and layout-triggering animation properties.
