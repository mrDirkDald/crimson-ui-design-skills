...
This HARD handbook chapter is a deep operational guide for **interaction craft**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Specify hover, focus, pressed, drag, select, expanded, loading, success, error, disabled, and recovery behavior for meaningful controls.
2. Make pointer delight supplemental; core interaction must survive keyboard, touch, reduced motion, and accessibility APIs.
3. Use custom cursors only when they communicate interaction or brand meaning and never hide system affordances without replacement.
4. Ensure scroll-driven effects never steal scroll control or trap users.
5. Design transitions to preserve orientation and state, not merely decorate route changes.
