# High-craft web QA master pass

## Purpose

This HARD handbook chapter is a deep operational guide for **high-craft web qa master pass**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Capture representative screenshots at multiple widths and compare whole-page silhouette before pixel-level details.
2. Verify narrative order, visual anchor, proof placement, CTA hierarchy, type rhythm, media consistency, and motion grammar.
3. Run anti-template tests and inspect whether the page still expresses the product with branding temporarily hidden.
4. Verify semantic controls, keyboard, reduced motion, overflow, zoom, loading/error/degraded states, and media failure.
5. Finish with one bounded confirmation pass after batch fixes; do not enter endless cosmetic iteration.
