...
This HARD handbook chapter is a deep operational guide for **web component composition**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Model components around stable semantic units rather than arbitrary visual boxes.
2. Watch boolean-prop proliferation and impossible state combinations; prefer explicit variants or composition when it clarifies ownership.
3. Keep layout primitives distinct from product components so page-level art direction is not trapped by generic card APIs.
4. Preserve semantic HTML and keyboard behavior through abstraction.
5. Use visual-regression and component-state harnesses for shared components that affect many surfaces.
