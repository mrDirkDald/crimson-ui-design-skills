...
This HARD handbook chapter is a deep operational guide for **typography craft**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Treat type as a layout system with roles, measure, line breaks, optical size, numerals, and language coverage.
2. Hand-tune important hero and campaign line breaks at representative widths instead of trusting one fluid size formula.
3. Use contrast in family, weight, width, case, tracking, and scale intentionally; avoid using size alone for hierarchy.
4. Keep body measure and line height optimized for reading even when display typography is expressive.
5. Verify font loading, fallback metrics, layout shift, diacritics, Cyrillic/Latin coverage, and variable-axis behavior.
