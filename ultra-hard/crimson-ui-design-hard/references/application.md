...
## Application layout archetype before cards

Identify the work model before choosing surfaces.
...
- canvas + properties;
- queue + details;
- command bar + work area;
- device visualizer + contextual controls;
- split pane.

Do not default to sidebar + dashboard cards.

### Device configurator rule
...
+ compact profile/service controls
```

over a dashboard made from summary cards.

### Density by frequency

High-frequency operational controls may be compact.
Rare settings may be more spacious.
Do not apply identical card padding/radius/spacing everywhere.
