...
For meaningful redesign where several structures are plausible, load `references/design-deliberation.md`.

Do not jump directly from diagnosis to the first attractive composition.

The chosen visual direction should be able to answer:
- why this structure;
- why this hierarchy;
- why this density;
- why this identity device;
- why this motion level;
- why this direction is stronger than the best alternative for this product.

If the user already approved a direction, preserve it unless evidence shows a material usability/accessibility/platform problem.

## Role-based typography grammar

Multiple typefaces are justified only when they have stable jobs.

Possible roles:
- display/brand;
- section heading;
- body;
- metadata/technical/mono.

Do not mix several typefaces merely for variety.

A useful contrast system can combine:
- expressive display;
- condensed/structured headings;
- neutral readable body;
- mono for technical/status metadata.

But every role must remain readable and consistent.

## Recurrent visual grammar

Prefer a small repeated visual grammar over a new effect in every section.

Potential recurring devices:
- one section marker;
- one metadata treatment;
- one image treatment;
- one accent behavior;
...
Use repetition to build identity.

Do not stack all possible motifs.

## Media normalization and local contrast

When source imagery is heterogeneous, create a coherent art direction through controlled treatment:
- crop/focal rules;
- grayscale or color grading where appropriate;
- contrast/brightness normalization;
- texture only if product-grounded;
- consistent overlay logic.

For text over media, prefer **local/directional contrast control**:
- directional gradient;
- localized radial shade;
- text-safe zone;
- crop change.

Do not automatically cover the whole image with one heavy black overlay.

## Attention budget

Autoplay reels, marquees, blinking indicators, pulse CTAs, animated counters, parallax and carousels all compete for attention.

For each viewport define:
```text
PRIMARY ATTENTION TARGET:
SECONDARY MOTION:
QUIET / BACKGROUND MOTION:
PAUSED / SUPPRESSED WHEN:
```

Do not let several autonomous effects compete continuously.

A polished interface may contain rich motion while still having one dominant attention target.

## Contextual persistent action

A sticky/persistent CTA or action is strongest when it appears because the original action is no longer visible and disappears/yields when the user reaches the destination.

Before making an action persistent, ask:
- is it still relevant throughout this section;
- is the original control already visible;
- is the destination already on screen;
- does it cover important content;
- should it disappear near the target.

Persistence is contextual, not a default badge of importance.

## Autonomous media contract

Autoplay media or carousels should define:
- current item/chapter;
- progress where useful;
- pause/mute;
- manual controls;
- user-interaction pause behavior;
- resume behavior;
...
Hover-only disclosure is an enhancement, not a complete interaction model.

If essential content/actions are revealed on hover, provide appropriate:
- keyboard;
- touch;
- controller/remote

access where those inputs are relevant.

## Semantic control gate

Use semantic controls for interactive behavior:
- `button` for action;
- link for navigation;
- checkbox/switch for binary state;
- radio/listbox/select for exclusive choice;
- tab for peer-panel switching.

Do not use a styled `div`/`span` as an interactive control when a semantic element fits.

Custom controls must recreate keyboard, focus, state and accessibility semantics.

## Success integrity

Do not show success before the real operation succeeds.

Production action state should normally follow:
```text
IDLE → VALIDATING → SUBMITTING → SUCCESS / FAILURE
```

A local submit handler is not evidence that a backend request succeeded.

Static prototypes may simulate success only when that simulation is clearly understood as prototype behavior.

## Reference extraction discipline

When learning from a reference, record:
...
Extract principles rather than cloning the reference's palette, typography, effects or layout literally.

## SVG icon direction

### Lucide baseline

When there is no coherent existing project icon family, use **Lucide Icons as the default baseline**.

Priority:
1. existing coherent project icons;
2. Lucide;
3. compatible custom SVG for brand/domain-specific concepts.

Keep one icon language per surface. Preserve Lucide's round stroke character, optical sizing and `currentColor` behavior unless the product design system intentionally normalizes it differently.

Do not force Lucide for brand marks or inaccurate domain concepts.
...
## Anti-cardification and surface economy

When composition starts to resemble a generic AI dashboard, load `references/anti-neuroslop-layout.md`.

Prefer grouping in this order:

```text
alignment → proximity → typography → whitespace → value/color → divider → container
```

A card/panel should communicate a real boundary.

### Whole-screen silhouette

A strong application should reveal:
- dominant workspace;
- support/inspector;
- navigation;
- state/status.

If the screen reads as equal rectangular blocks, hierarchy is weak.

### Product-object dominance

For product-specific tools, the primary object/workspace should dominate:
- controller configurator → controller/mapping;
- editor → canvas/document;
- downloader → queue;
- monitor → live data/events.

Do not make generic dashboard chrome more important than the product itself.

### Container necessity

Before each visible panel ask:

```text
WHAT SEMANTIC OR INTERACTION BOUNDARY DOES THIS CREATE?
```

If the answer is only visual separation, try spacing/alignment/type first.


## Design operating dials

For meaningful REDESIGN, set these independently before coding:
...
These are decision controls, not style presets.

Use subject, audience, frequency of use, platform, input method and trust/accessibility constraints to choose them.

A repeated-use desktop utility may reasonably be `low/low/compact`; an expressive campaign may be `high/high/spacious`; many products sit between these extremes.

Do not allow one dial to silently control the others.

## Characteristic-anchor gate

The first dominant visual anchor should come from the product's own world: its object, workflow, content, data decision, media, or interaction.

Before using a generic hero treatment ask:

```text
WHAT IS THE MOST CHARACTERISTIC THING ABOUT THIS PRODUCT/SUBJECT?
CAN THAT BECOME THE PRIMARY ANCHOR?
```

Avoid defaulting to stat cards, gradients, abstract blobs or decorative metrics when they do not teach the user what the product is.

## Pattern inheritance filter

When adapting a reference skill/site, separate:
...
Do not globally inherit rigid font/icon/palette/layout bans from another design system.

A reference can be excellent and still be wrong for this product.
