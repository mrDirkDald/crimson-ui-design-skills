---
name: crimson-ui-electron-preload-ipc
description: Deep aspect subskill for preload and ipc within the Crimson UI Design MAX system.
---
...
Keep renderer UX responsive while respecting Electron process boundaries.
...
Expose narrow typed capabilities through preload rather than leaking broad Node access.
...
Model async IPC states explicitly in the UI.
...
Handle cancellation, timeout, duplicate requests, and stale responses where applicable.
...
Do not block the renderer on long-running native work.
...
Keep security-sensitive validation on the privileged side.
...
- [ ] cancellation/timeout considered
- [ ] renderer nonblocking
- [ ] privileged validation retained
