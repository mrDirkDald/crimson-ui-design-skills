---
name: crimson-ui-design-system-governance-drift
description: Deep aspect subskill for governance and drift detection within the Crimson UI Design MAX system.
---
...
Detect and prevent token, component, visual, and naming drift across a growing product.
...
Search for near-duplicate raw values, repeated local overrides, and parallel components solving the same problem.
...
Prefer fixing the smallest governing layer that actually owns the drift.
...
Do not centralize a value merely because it repeats twice if the semantics differ.
...
Document deprecation paths for replaced components/tokens.
...
Use visual regression and usage inventory for high-blast-radius changes.
...
- [ ] drift inventory
- [ ] governing owner identified
- [ ] false duplication avoided
