---
name: crimson-ui-tauri-capabilities-security
description: Deep aspect subskill for capabilities and security within the Crimson UI Design MAX system.
---
...
Keep Tauri permissions/capabilities aligned to the minimum real product need.
...
Request only capabilities required by the current feature set.
...
Do not widen filesystem/process/shell access to simplify frontend code.
...
Keep untrusted content separated from privileged commands.
...
Treat denial/unavailability as normal product state with useful recovery.
...
Review plugin permissions when adding new native integrations.
