# Desktop Window / Lifecycle Audit

## Purpose

Use this audit as a dedicated HARD pass when the risk area is material.

## Evidence

Capture the target surface, user path, relevant states, governing code/components, and runtime constraints before judging.

## Audit sequence

1. Reproduce the real state.
2. Inspect hierarchy and semantics before decorative polish.
3. Test normal plus edge/failure/recovery states.
4. Test keyboard/focus/input and scaling where relevant.
5. Separate BLOCKER/HIGH/MEDIUM/LOW issues.
6. Fix root causes in a batch.
7. Run one confirmation pass.

## Failure patterns

- state is visually ambiguous or dependent on color only;
- control looks interactive but is not, or vice versa;
- error/recovery loses valid user work;
- visual hierarchy changes unpredictably between states;
- content overflows or becomes unreadable under scaling/localization;
- the result is declared correct without runtime evidence.
