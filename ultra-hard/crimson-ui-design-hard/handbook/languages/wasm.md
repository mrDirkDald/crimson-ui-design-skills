# WebAssembly UI Implementation Guide — HARD

## Purpose

Use this guide when UI code is materially implemented in **WebAssembly**. Typical ecosystems include **Rust/C/C++/other code embedded in browser UI**. The language alone does not determine the UI architecture; inspect the actual toolkit and version before applying APIs.

## Primary engineering concern

Pay special attention to **DOM bridge ownership, transfer cost, accessibility outside canvas, progressive fallback**.
