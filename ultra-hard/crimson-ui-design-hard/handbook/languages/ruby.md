# Ruby UI Implementation Guide — HARD

## Purpose

Use this guide when UI code is materially implemented in **Ruby**. Typical ecosystems include **Rails/Hotwire/Turbo and server-rendered web**. The language alone does not determine the UI architecture; inspect the actual toolkit and version before applying APIs.

## Primary engineering concern

Pay special attention to **server-driven navigation, form/error state, progressive enhancement, asset pipeline**.
