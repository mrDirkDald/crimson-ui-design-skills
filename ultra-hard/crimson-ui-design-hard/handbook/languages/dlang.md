# D UI Implementation Guide — HARD

## Purpose

Use this guide when UI code is materially implemented in **D**. Typical ecosystems include **native toolkit bindings, custom renderers and desktop utilities**. The language alone does not determine the UI architecture; inspect the actual toolkit and version before applying APIs.

## Primary engineering concern

Pay special attention to **GC/native boundary, event loop, toolkit-specific conventions**.
