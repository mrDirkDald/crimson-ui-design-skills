# OCaml UI Implementation Guide — HARD

## Purpose

Use this guide when UI code is materially implemented in **OCaml**. Typical ecosystems include **Reason/Rescript-adjacent web, native bindings, terminal/tool UIs**. The language alone does not determine the UI architecture; inspect the actual toolkit and version before applying APIs.

## Primary engineering concern

Pay special attention to **state/effect boundaries, JS/native interop, packaging**.
