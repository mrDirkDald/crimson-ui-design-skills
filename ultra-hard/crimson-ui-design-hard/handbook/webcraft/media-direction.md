...
This HARD handbook chapter is a deep operational guide for **media and asset direction**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Choose imagery because it proves or embodies the product, not because a dark abstract render fills space.
2. Define crop rules, focal points, aspect ratios, masking, texture, and background treatment as a coherent system.
3. Avoid one identical overlay applied to every image; use local contrast where text actually needs support.
4. For product screenshots, decide whether they are evidence, demonstration, background texture, or decorative object and style accordingly.
5. Test asset substitution: if any stock image can replace the current one without changing the design thesis, art direction is weak.
