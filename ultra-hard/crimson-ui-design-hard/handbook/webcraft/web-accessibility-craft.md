# Accessibility in expressive web

## Purpose

This HARD handbook chapter is a deep operational guide for **accessibility in expressive web**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Expressive composition still requires semantic landmarks, headings, focus order, control names, and readable contrast.
2. Do not let visual order diverge dangerously from DOM/focus order.
3. Provide reduced-motion behavior that remains designed rather than simply broken or empty.
4. Ensure text over media has local contrast support and survives image loading failure.
5. Test keyboard, screen-reader naming, zoom, text scaling, and forced/high-contrast modes where relevant.
