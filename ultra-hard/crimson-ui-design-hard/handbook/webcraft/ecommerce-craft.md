...
This HARD handbook chapter is a deep operational guide for **e-commerce craft**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Let product imagery, selection, price, variant state, stock, delivery, returns, and purchase action dominate visual hierarchy.
2. Do not let campaign art direction obscure variant selection, availability, price, or purchase consequences.
3. Keep product media consistent enough for comparison while allowing hero storytelling where it adds value.
4. Design loading, unavailable variants, cart errors, discount state, and mobile sticky actions explicitly.
5. Treat trust, shipping, returns, and sizing information as decision support rather than footer clutter.
