...
This HARD handbook chapter is a deep operational guide for **seo and semantic structure**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Keep one coherent document outline and meaningful landmarks even when visual composition is unconventional.
2. Use actual text for important content; do not hide core meaning inside canvas/WebGL imagery only.
3. Preserve canonical metadata, titles, descriptions, structured data, and crawlable links when relevant.
4. Avoid duplicating headings or hidden keyword text for visual convenience.
5. Performance, semantic HTML, and accessible navigation generally support both search and human usability.
