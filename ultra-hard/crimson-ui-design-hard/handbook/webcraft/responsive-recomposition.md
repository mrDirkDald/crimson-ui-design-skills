...
This HARD handbook chapter is a deep operational guide for **responsive recomposition**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Treat mobile as a separate art-direction problem, not desktop content stacked vertically.
2. Re-evaluate order, crop, line breaks, interaction density, navigation, motion, and proof placement at each breakpoint class.
3. Preserve the visual thesis even when the hero layout, media ratio, or interaction model must change completely.
4. Design for content expansion, localization, browser chrome, safe areas, and dynamic viewport height.
5. Verify touch ergonomics and keyboard behavior independently of pointer hover states.
