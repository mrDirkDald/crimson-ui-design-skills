# Brand systems on the web

## Purpose

This HARD handbook chapter is a deep operational guide for **brand systems on the web**. Use it together with `references/web-craft-analysis.md` and the platform/stack guide that actually governs the implementation.

## Detailed rules

1. Translate brand attributes into repeatable web rules rather than pasting a logo onto a generic layout.
2. Define type roles, color roles, image treatment, geometry, icon language, motion behavior, and tone of voice as one grammar.
3. Allow controlled exceptions for hero or campaign moments while keeping core interaction predictable.
4. Separate brand expression from semantic status colors and accessibility requirements.
5. Test the system across empty, dense, error, account, settings, and utility states, not only the marketing hero.
