...
Use semantic HTML first. Establish CSS custom properties for semantic tokens, logical properties for RTL, Grid/Flexbox according to structure, media/container queries where useful, and minimal JS for state that CSS/HTML cannot represent. Preserve focus outlines or replace with equally visible focus-visible styling. Use inline/external SVG or Lucide assets rather than emoji controls.
