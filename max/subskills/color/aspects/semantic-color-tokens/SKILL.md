---
name: crimson-ui-color-semantic-color-tokens
description: Deep aspect subskill for semantic color tokens within the Crimson UI Design MAX system.
---
...
Map colors to stable interface meaning so components do not invent local palettes.
...
Define canvas, surfaces, text roles, borders, primary accent, optional supporting accent, focus, and semantic states.
...
Separate raw palette scales from semantic tokens and component aliases.
...
Do not let component-local hex values bypass the system without a documented reason.
...
Keep brand color distinct from danger/success semantics when collisions would cause ambiguity.
...
Ensure disabled and subtle states are still perceivable without becoming active-looking.
...
- [ ] raw palette separated
- [ ] component aliases coherent
