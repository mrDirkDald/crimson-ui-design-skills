...
# MAX Edition Design-System Addendum

When changing shared design primitives, build an impact map of consuming components and surfaces. Validate theme parity, state parity, accessibility, and visual regression across representative consumers. Avoid fixing local drift by adding one more token when an existing semantic role can be corrected.
