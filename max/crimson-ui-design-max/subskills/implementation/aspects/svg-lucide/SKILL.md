---
name: crimson-ui-implementation-svg-lucide
description: Deep aspect subskill for svg and lucide implementation within the Crimson UI Design MAX system.
---

# SVG and Lucide Implementation

## Purpose

Implement consistent accessible icons across web and app renderers.
...
Use existing project icons first; otherwise default to Lucide where available.
...
Import only needed icons when the library/toolchain supports tree-shaking.
...
Use currentColor for inherited control color when appropriate.
...
Mark decorative SVGs hidden from assistive technology and label icon-only controls at the control level.
...
Normalize custom SVG viewBox, stroke/fill, baseline, and theme behavior before mixing with Lucide.
...
- [ ] no emoji icons
- [ ] imports scoped
- [ ] currentColor appropriate
- [ ] a11y semantics correct
- [ ] custom SVG normalized
