...
## Design choice QA gate

For meaningful REDESIGN, verify the chosen direction was selected deliberately rather than being the first plausible idea.

Check:
- at least one credible alternative was considered unless direction was user-specified;
- the selected direction has a stated product-fit advantage;
- its main tradeoff is acknowledged;
- identity comes from system-level choices, not only decoration;
- runtime proof criteria were defined.

Do not fail trivial fixes for not generating concepts.

## Continuous motion QA

When several continuous/automatic effects coexist, check whether they compete for attention or ignore reduced-motion needs.

Typical risk stack:
- marquee + pulse + autoplay reel + parallax + blinking indicator + auto carousel.

Reduce, pause, sequence, or subordinate effects until one attention hierarchy is clear.

## Semantic-control QA

FAIL when important interactivity is implemented as generic clickable text/container without required semantic/keyboard/focus/state behavior.

Also verify hover disclosures have touch and keyboard equivalents when the hidden content matters.

## Reference-integrity web QA

Run when `web-reference-corpus.md` influenced the design.

Check:
- each reference answered a concrete question;
- award/gallery bias was not treated as universal UX evidence;
- mechanisms were adapted rather than styling copied;
- the page has one coherent grammar rather than a reference collage;
- section rhythm is intentionally varied where useful;
- saturated motifs have a product-specific reason;
- logo-removal, section-shuffle and asset-substitution tests do not expose generic structure;
- mobile preserves hierarchy/thesis rather than desktop coordinates.

MEDIUM when the page is polished but generic/reference-collaged. HIGH when reference copying damages usability, accessibility, product truth or runtime behavior.
