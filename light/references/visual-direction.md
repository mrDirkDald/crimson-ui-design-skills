...
Identity can come from a small set of repeated compositional rules rather than from adding a unique effect to every section.

Useful recurring grammar includes:
- stable section-marker treatment;
- controlled display/body/technical type roles;
- one coherent image treatment;
- repeated accent behavior;
- one border/corner/notch motif;
- consistent metadata formatting;
- one motion/easing family.

The repeated rule should make the product easier to recognize or scan.

Do not turn repetition into decoration spam. One motif repeated with restraint is stronger than six unrelated signature tricks.

### Role-based font families

Multiple font families can be valid when each has a durable role such as display, heading/control, body, and technical/mono metadata.

Do not add extra families merely for variety. If two families serve the same role, consolidate unless the brand system clearly benefits from the distinction.

### Media normalization

A controlled image treatment such as grayscale, desaturation, tinting, consistent contrast, or crop can unify heterogeneous content and allow one accent family to carry interaction hierarchy.

Do not destroy subject detail or skin/content fidelity merely to force a style.
